...
import { MusicNotes, Plus, Trash, X } from 'phosphor-react';
...
const MediaShelf: React.FC = () => {
  ...
  const onDragStart = (e: React.DragEvent, item: any, type: 'image' | 'audio') => {
    ...
    if (type === 'image') {
      e.dataTransfer.setData('itemUrl', item.objectUrl);
    } else {
  ...
  return (
    <div className="w-72 bg-[var(--color-bg-panel)] border-r border-[var(--color-border-default)] flex flex-col animate-in slide-in-from-left duration-300 relative z-20">
